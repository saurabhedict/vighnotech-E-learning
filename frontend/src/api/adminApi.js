...
async function prepareResource(resourceData, onProgress) {
  const { type, title, file, url, quizJson, opts, price, identifier, linkExternal } = resourceData
  ...
  return {
    ...
    // 'link' open mode (secure proxy vs open-in-new-tab).
    ...(linkExternal !== undefined ? { linkExternal: !!linkExternal } : {}),
  }
}
